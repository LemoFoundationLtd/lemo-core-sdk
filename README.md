...
3. `object` - Candidate information which is very similar with [deputy](#data-structure-deputyNode). But there is no `votes` field in candidate information. And `isCandidate` field is added, it is used to register or unregsiter candidate
