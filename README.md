...
- `sig` Signature data. The signature field is 65 characters long.
- `gasPayerSig` Paid gas transaction signature data. 65 characters in length
...
lemo.TxType.TRANSFER_ASSET | 7 | Trading assets
...
```
- `category` Asset type
- `decimals` The decimal place of the issued asset, which indicates how many digits are reduced to the decimal point. The default is 18 digits.
- `totalSupply` The total amount of assets issued, when the assets are issued and destroyed, the total amount of assets will change in real time, the total amount of assets issued is `issuance*10^decimals`
- `isReplenishable` Whether the asset can be replenish, The asset cannot be modified again after set， set when the asset is created, the default is `true`
- `isDivisible` Whether the asset is divisible, set when the asset is created, the default is `true`
- `issuer` The publisher address, based on the signature field at the time the asset was created
- `profile` Additional information about the asset
    - `name` The name of the asset when it was created
    - `symbol` Asset identification
    - `description` Basic information of assets
    - `suggesteGasLimit` Trading consumption limit, and ` gasLimit ` same usage, creating assets set by users themselves, the default is 60000
    - `stop` Whether to stop the special deals, the default is ` false `, if there is a loophole, set it to ` true ` can stop any special deal

<a name="data-asset-category"></a>

category | description
--- |---
1 |  TokenAsset, ERC20 for ethereum
2 |  NonFungibleAsset, ERC721 for ethereum
3 |  CommonAsset, Divisible assets, merging the above two scenarios
