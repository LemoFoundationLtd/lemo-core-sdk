...
describe('lemoClient.lemo.contract', function() {
    it('should create simple contract with one method from abi with explicit type name', function () {
        ...
        var myCon = lemoClient.lemo.contract(description).at(address);
    ...
    it('should create contract with nondefault constructor', function (done) {
        ...
        provider.injectValidation(function (payload) {
            if (steps === 1) {
                ...
                assert.equal(payload.jsonrpc, '2.0');
                assert.equal(payload.method, 'lemo_newBlockFilter');
                steps++;
            }
        });

        lemoClient.lemo.contract(description).new(2, {from: address, data: code}, function(e, myCon){
                done();
                // lemoClient.stopWatching();
        });
    });
});
