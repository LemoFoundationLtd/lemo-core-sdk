...
export default class  {
    ...
    */
    subscribe(requester, withBody, callback) {
        ...
        const getWithBody = () => {
            const withBodyMap = new Map()
            withBodyMap.set(withBody, callback)
            return withBodyMap
        }
        this.callbacks[newWatchId] = getWithBody()
        if (Object.keys(this.callbacks).length === 1) {
            Object.keys(Object.values(this.callbacks)).some(item => {
                if (item === 'true') {
                    this.withBody = withBody
                    this.watchId = requester.watch(`${CHAIN_NAME}_latestStableBlock`, [!!this.withBody], this.watchHandler.bind(this))
                    return true
                }
