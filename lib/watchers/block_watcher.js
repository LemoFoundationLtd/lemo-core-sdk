...
export default class  {
    ...
    */
    subscribe(requester, withBody, callback) {
        ...
        const startHeight = this.getLatestBlockHeight() // 最开始将通知出去的块高度
    ...
    */
    ...
    notify(block) {
        Object.values(this.callbackInfos).forEach((item) => {
            if (item.startHeight >= block.header.height) {
                return
            }
            if (!item.withBody) {
                item.callback({header: block.header})
            }
            item.callback(block)
        })
    }
    ...
    */
    isExistBlock(block) {
        if (this.lastBlockHeight === -1 || this.pendingBlocks.findIndex(item => item.header.height === block.header.height) === -1
