...
export default class  {
    ...
    */
    subscribe(requester, withBody, callback) {
        ...
        const tempWithBody = withBody
        this.callbacks[newWatchId] = {withBody, callback}
        ...
        } else if (tempWithBody !== this.getWidthBody()) {
    ...
    */
    ...
    getWidthBody() {
        return  Object.values(this.callbacks).some(item => {
    ...
    */
    processBlock (fetchBlock, block)  {
        ...
        if (this.pendingBlocks.indexOf(block.header.height) === -1 && this.lastBlockHeight !== block.header.height
            && this.lastBlockHeight !== -1) { // 过滤相同的块
            return
        }
