...
export default class  {
    ...
    */
    subscribe(withBody, callback) {
        ...
        if (Object.keys(this.callbackInfos).length === 1) {
            this.watchId = this.requester.watch(`${CHAIN_NAME}_latestStableBlock`, [!!this.getWidthBody()], this.watchHandler.bind(this))
    ...
    * @param {Object} block
    */
    watchHandler(block, watchId, error) {
        if (error) {
    ...
    notify(block, error) {
        Object.values(this.callbackInfos).forEach((item) => {
            if (error) {
                throw new Error(errors.InvalidPollTxTimeOut())
            }
            if (!item.withBody) {
