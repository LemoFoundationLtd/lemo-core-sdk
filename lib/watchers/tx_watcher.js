...
export default class {
     ...
     * @param {string|number} expirationTime Transaction expiration time
    ...
    waitTx(txHash, expirationTime) {
        if (this.serverMode) {
            ...
            return this.waitTxByGetTxByHash(txHash, expirationTime)
        }
    }
    ...
    waitTxByWatchBlock(txHash, expirationTime) {
        return new Promise((resolve, reject) => {
            ...
            const expirationId = setTimeout(() => {
                this.requester.unsubscribe(subscribeId)
                reject(new Error(errors.InvalidTxTimeOut()))
            }, time)
        })
    }

    waitTxByGetTxByHash(txHash, expirationTime) {
        return new Promise((resolve, reject) => {
