...
class Watcher {
    ...
    subscribe(requester, callback, withBody) {
        ...
        const newWatcherId = this.idGenerator++
        this.callbacks = new Map().set(this.idGenerator, callback)
        if (this.callbacks.length === 1) {
            this.watcherId = requester.watch(`${CHAIN_NAME}_latestStableBlock`, [!!withBody], this.watchHandler.bind(this))
        }
        return  newWatcherId
    }
    ...
    unSubscribe(watcherId) {
        ...
        this.callbacks = new Map().set(this.idGenerator, {})
        if (!this.callbacks.length) {
            this.requester.stopWatch(watcherId)
        }
    }
