...
class Watcher {
    ...
    subscribe(requester, callback, withBody) {
        ...
        this.newWatcherId = this.idGenerator++
        this.callbacks.push(callback)
        if (this.callbacks.length === 1) {
            requester.watch(`${CHAIN_NAME}_latestStableBlock`, [!!withBody], this.watchHandler.bind(this))
        }
        return  this.newWatcherId
    }
    ...
    unSubscribe(callback) {
        ...
        this.callbacks.splice(this.newWatcherId - 1, 1)
        if (!this.callbacks.length) {
            this.requester.stopWatch(this.newWatcherId - 1)
        }
    }
