...
class LemoClient {
    constructor(config = {}) {
        ...
        this._blockWatcher = new BlockWatcher()
...
function createModule(lemo, moduleName, apis) {
    Object.entries(apis).forEach(([key, value]) => {
        ...
        new Api(apiConfig, {requester: lemo._requester, chainID: lemo.config.chainID, blockWatcher: lemo._blockWatcher}).attachTo(lemo, moduleName)
    })
}
...
function createAPI(lemo, moduleName, apiName, methodNameOrFunc) {
    ...
    new Api(config, {requester: lemo._requester, chainID: lemo.config.chainID, blockWatcher: lemo._blockWatcher}).attachTo(lemo, moduleName)
}
