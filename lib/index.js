...
function createModule(lemo, moduleName, apis) {
    Object.entries(apis).forEach(([key, value]) => {
        ...
        new Api(apiConfig, lemo._requester, lemo.config.chainID).attachTo(lemo, moduleName)
    })
}
...
function createAPI(lemo, moduleName, apiName, methodNameOrFunc) {
    ...
    new Api(config, lemo._requester, lemo.config.chainID).attachTo(lemo, moduleName)
}
