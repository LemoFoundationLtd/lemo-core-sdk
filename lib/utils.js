...
export function createTempAddress(from, userId) {
    if (typeof userId === 'string') {
        const result = Buffer.from(userId).toString('hex')
        if (result.length > USER_ID_LENGTH) {
            throw new Error(errors.TXInvalidUserIdLength())
        }
        userId = result.padStart(USER_ID_LENGTH, '0')
    } else {
        throw new Error(errors.TXInvalidUserIdType())
    }
