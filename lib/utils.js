...
export function parseBlock(block) {
    block.ChangeLogs[0].type = parseChangeLogType(block.ChangeLogs[0].type)
    return block
}
...
export function formatMoney(mo) {
    ...
    if (/0{18}$/.test(mo)) {
        ...
        return `${mo.slice(0, mo.length - 9)}GMo`
    } else if (/0{6}$/.test(mo)) {
        return `${mo.slice(0, mo.length - 6)}MMo`
    } else if (/0{3}$/.test(mo)) {
        return `${mo.slice(0, mo.length - 3)}KMo`
    } else {
        return `${mo}Mo`
    }
}
