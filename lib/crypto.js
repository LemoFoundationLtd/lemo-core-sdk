...
function privateToAddress(privKey) {
    privKey = (typeof privKey === 'string') ? Buffer.from(privKey, 'hex') : privKey
    return pubKeyToAddress(privateToPublic(privKey))
}

function randomBytes(size) {
    const numArr =  Array(size).fill(0).map(() => Math.floor(Math.random() * 256))
