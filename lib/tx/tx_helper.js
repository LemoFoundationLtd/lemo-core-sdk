...
export function verifyReplenishAssetInfo(config) {
    ...
    if (config.replenishAmount === undefined) {
        throw new Error(errors.TXIsNotDecimalError('replenishAmount'))
    }
    checkNegative(config, 'replenishAmount')
    checkType(config, 'replenishAmount', ['string'], true)
    if (/^0x/i.test(config.replenishAmount)) {
        throw new Error(errors.TXIsNotDecimalError('replenishAmount'))
    }
}
...
export function verifyModifyAssetInfo(config) {
    ...
    if (config.info.stop) {
        if (typeof config.info.stop === 'boolean') {
            checkType(config.info, 'stop', ['boolean'], false)
        } else if (typeof config.info.stop === 'string' && (config.info.stop === 'true' || config.info.stop === 'false')) {
            checkType(config.info, 'stop', ['string'], false)
        } else {
            throw new Error(errors.TxInvalidSymbol())
        }
    }
}
...
export function verifyTransferAssetInfo(config) {
    ...
    if (config.transferAmount === undefined) {
        throw new Error(errors.TXIsNotDecimalError('transferAmount'))
    }
    checkNegative(config, 'transferAmount')
    checkType(config, 'transferAmount', ['string'], true)
    if (/^0x/i.test(config.transferAmount)) {
        throw new Error(errors.TXIsNotDecimalError('transferAmount'))
    }
}
