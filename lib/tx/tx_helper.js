...
export function verifyTxConfig(config) {
    ...
    if (config.data) {
        checkFieldType(config, 'data', ['string', 'object'], false)
    }
...
export function verifyBoxTXInfo(boxTxInfo) {
    checkFieldType(boxTxInfo, 'subTxList', ['array'], false)
}

export function verifyContractCreationInfo(code, constructorArgs) {
    checkType(code, 'code', ['string'], false)
    checkType(constructorArgs, 'constructorArgs', ['string'], false)
}
