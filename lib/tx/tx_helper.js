...
export function verifyTxConfig(config) {
    ...
    if (config.sigs) {
        checkFieldType(config, 'sigs', ['object'], false)
    ...
    if (config.gasPayerSigs) {
        checkFieldType(config, 'gasPayerSigs', ['object'], false)
...
export function verifymodifySignersInfo(modifySignersInfo) {
    checkFieldType(modifySignersInfo, 'signers', ['object'], false)
    modifySignersInfo.signers.forEach((signer, index) => {
        ...
        checkType(signer.weight, `weight[${index}].weight`, ['number'], true)
        checkNegative(signer.weight, `weight[${index}].weight`)
    })
}
