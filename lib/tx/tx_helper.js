...
export function verifyIssueAssetInfo(config) {
    ...
    checkType(config, 'amount', ['number'], true)
    checkRange(config, 'amount', 0, 0xffff)
}

export function verifyTradingAssetInfo(config) {
    checkType(config, 'value', ['number'], true)
    checkRange(config, 'value', 0, 0xffff)
    checkType(config, 'input', ['string'], false)
}
