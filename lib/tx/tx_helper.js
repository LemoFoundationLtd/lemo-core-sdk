...
export function verifyTxConfig(config) {
    ...
    checkType(config, 'from', ['string'], false)
    ...
    if (config.sigs) {
        checkType(config, 'sigs', ['Array'], false)
        config.sigs.forEach((sigs, index) => {
            checkType(config.sigs, sigs[index], ['string'], false)
            checkMaxBytes(config.sigs, sigs[index], TX_SIG_BYTE_LENGTH)
        })
    }
    if (config.gasPayerSigs) {
        checkType(config, 'gasPayerSigs', ['Array'], false)
        config.gasPayerSigs.forEach((gasPayerSigs, index) => {
            checkType(config.gasPayerSigs, gasPayerSigs[index], ['string'], false)
            checkMaxBytes(config.gasPayerSigs, gasPayerSigs[index], TX_SIG_BYTE_LENGTH)
        })
    }
}
...
export function verifyModifyAssetInfo(config) {
    ...
    if (config.updateProfile.name) {
        checkType(config.updateProfile, 'name', ['string'], false)
    }
    if (config.updateProfile.symbol) {
        checkType(config.updateProfile, 'symbol', ['string'], false)
    }
    if (config.updateProfile.description) {
        checkType(config.updateProfile, 'description', ['string'], false)
        checkMaxLength(config.updateProfile, 'description', 256)
    }
    if (config.updateProfile.suggestedGasLimit) {
        checkType(config.updateProfile, 'suggestedGasLimit', ['string'], true)
    }
    if (config.updateProfile.freeze) {
        checkType(config.updateProfile, 'freeze', ['boolean', 'string'], false)
...
export function verifymodifySignersInfo(modifySignersInfo) {
    checkType(modifySignersInfo, 'signers', ['Array'], false)
    modifySignersInfo.signers.forEach((signers, index) => {
        checkType(modifySignersInfo.signers, signers[index], ['string'], false)
        checkMaxBytes(modifySignersInfo.signers, signers[index], TX_SIG_BYTE_LENGTH)
    })
}
