...
export function verifyTxConfig(config) {
    ...
    if (config.sigs) {
        ...
        config.sigs.forEach(sigs => {
            checkType(sigs, 'sigs', ['string'], false)
            checkMaxBytes(sigs, 'sigs', TX_SIG_BYTE_LENGTH)
        })
    }
    if (config.gasPayerSigs) {
        checkType(config, 'gasPayerSigs', ['Array'], false)
        config.gasPayerSigs.forEach(gasPayerSigs => {
            checkType(gasPayerSigs, 'gasPayerSigs', ['string'], false)
            checkMaxBytes(gasPayerSigs, 'gasPayerSigs', TX_SIG_BYTE_LENGTH)
        })
    }
}
...
export function verifymodifySignersInfo(modifySignersInfo) {
    ...
    modifySignersInfo.signers.forEach(signers => {
        checkType(signers, 'signers', ['string'], false)
        checkMaxBytes(signers, 'signers', TX_SIG_BYTE_LENGTH)
    })
}
