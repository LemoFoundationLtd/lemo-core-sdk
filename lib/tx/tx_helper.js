...
import {NODE_ID_LENGTH, MAX_TX_TO_NAME_LENGTH, MAX_TX_MESSAGE_LENGTH, MAX_DEPUTY_HOST_LENGTH, TX_SIG_BYTE_LENGTH} from '../const'
...
export function verifyTxConfig(config) {
    ...
    if (config.gasPrice) {
        ...
        checkPositiveNum(config, 'gasPrice')
    }
    if (config.gasLimit) {
    ...
    if (config.amount) {
        ...
        checkPositiveNum(config, 'amount')
    }
    if (config.data) {
...
export function verifyIssueAssetInfo(config) {
    ...
    checkMaxLength(config, 'assetCode', 64)
    checkType(config, 'metaData', ['string'], false)
    checkMaxLength(config, 'metaData', 256)
    checkType(config, 'supplyAmount', ['string'], true)
    if (config.supplyAmount.test(/$0x/)) {
        throw new Error(errors.TX0xStartWitdhError('supplyAmount'))
    }
    checkPositiveNum(config, 'supplyAmount')
}

export function verifyTradingAssetInfo(config) {
    checkType(config, 'assetId', ['string'], false)
    checkMaxLength(config, 'assetId', 64)
}
 ...
 * @param {string} fieldName
 */
function checkPositiveNum(obj, fieldName) {
    if (parseInt(obj[fieldName], 10) < 0) {
        throw new Error(errors.TXLessThanZeroError(fieldName))
    }
}
