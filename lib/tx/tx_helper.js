...
export function verifyCreateAssetInfo(config) {
    if (!config.category) {
        throw new Error('the category cannot be missing')
    }
    checkType(config, 'category', ['string', 'number'], true)
    ...
    checkType(config.profile, 'suggestedGasLimit', ['string', 'number'], true)
}
