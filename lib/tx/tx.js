...
export default class Tx {
     ...
     * @param {Array?} txConfig.sigs Signature hex data
     * @param {Array?} txConfig.gasPayerSigs Gas payer signature hex data
     */
    ...
    normalize(txConfig) {
        ...
        // sigs Array
        if (txConfig.sigs && typeof txConfig.sigs instanceof Array) {
            this.sigs.map(item => toHexStr(item))
        } else {
            this.sigs = []
        }
        // gasPayerSig Array
        if (txConfig.gasPayerSigs && typeof txConfig.gasPayerSigs instanceof Array) {
            this.gasPayerSigs.map(item => toHexStr(item))
        } else {
            this.gasPayerSigs = []
        }
        // from string
        ...
        } else if (isLemoAddress(txConfig.from)) {
     ...
     */
    signWith(privateKey) {
        this.sigs.push(new Signer().sign(this, privateKey))
    }
    ...
    serialize() {
        const raw = [
            ...
            toRaw(decodeAddress(this.from), 'from', TX_TO_LENGTH),
            ...
            toRaw(this.sigs, 'sigs'),
            toRaw(this.gasPayerSigs, 'gasPayerSigs'),
        ]
