...
import {DEFAULT_POLL_DURATION, MAX_POLL_RETRY, TX_POLL_DURATION, MAX_TIME_OUT} from '../config'
...
class Requester {
    ...
    watchTx(method, params, callback) {
        ...
        const poll = async () => {
            // console.log('poll', method)
            try {
                txByHash = await this.send(method, params)
                errCount = 0
            } catch (e) {
                console.error('watch fail:', e)
                if (++errCount <= this.maxPollRetry) {
        ...
        this.watchers[newWatchId] = setInterval(poll, TX_POLL_DURATION)
        // call first time immediately
        poll().catch(e => console.error(e))
        return newWatchId
    }
