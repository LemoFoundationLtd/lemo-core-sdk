...
export function parseAccount(account) {
    ...
    account.txCount = Number(account.txCount || 0)
...
export function parseChangeLogType(logType) {
    const dict = ['', 'BalanceLog', 'StorageLog', 'CodeLog', 'AddEventLog', 'SuicideLog', 'VoteForLog', 'VotesLog', 'CandidateProfileLog']
...
export function parseTxRes(signer, res) {
    ...
    tx.minedTime = Number(res.time)
    tx.blockHeight = Number(res.height)
...
export function parseTxListRes(signer, res) {
    ...
    txList = txList.map((item) => {
        ...
        tx.minedTime = Number(item.time)
        return tx
    })

    return {txList, total: Number(res.total || 0)}
}
