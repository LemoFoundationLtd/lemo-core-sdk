...
const apis = {
    ...
    async getAsset(assetCode) {
        ...
        return this.parser.parserAssetCode(result)
    },
    /**
     * 获取指定资产中保存的自定义数据
     ...
     * @return {Promise<object>}
     */
    async getMateData(assetId) {
        const result = await this.requester.send(`${ACCOUNT_NAME}_getMateData`, [assetId])
        return this.parser.parserMateData(result)
    },
}
