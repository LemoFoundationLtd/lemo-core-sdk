...
const notify = (signer, withBody, callback) => {
    for (let i = 0; i < pendingBlocks.length; i++) {
        callback(parseBlock(signer, pendingBlocks[i], withBody))
...
const apis = {
    ...
    async getCurrentBlock(stable, withBody) {
        ...
        return parseBlock(this.signer, block, withBody)
    },
    /**
    ...
    async getGenesis() {
        ...
        return parseBlock(this.signer, result, true)
    },
    /**
    ...
    watchBlock(withBody, callback) {
        const watchHandler = async block => {
            if (pendingBlocks.length) {
                const maxPblockHeight = Number(pendingBlocks[pendingBlocks.length - 1].header.height) + 1

                if (Number(block.header.height) !== maxPblockHeight) {
                    pendingBlocks.push(block)
                    for (let i = maxPblockHeight; i < Number(block.header.height); i++) {
                        const newBlock = await this.requester.send(`${MODULE_NAME}_getBlockByHeight`, [i, !!withBody])

                        pendingBlocks.some((item, index) => {
                            if (Number(newBlock.header.height) === Number(item.header.height) + 1) {
                                pendingBlocks.splice(index + 1, 0, newBlock)
                                return true
                            }
                        })
                        notify(this.signer, withBody, callback)
                    }
                } else {
                    pendingBlocks.push(block)
                    notify(this.signer, withBody, callback)
            ...
            pendingBlocks.push(block)
            callback(parseBlock(this.signer, block, withBody))
        }
