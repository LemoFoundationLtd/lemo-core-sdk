...
const apis = {
    ...
    watchBlock(withBody, callback) {
        const watchHandler = async block => {
            if (Object.keys(tempBlock).length) {
                const height = Number(tempBlock.header.height) + 1
                if (height !== Number(block.header.height)) {
                    const apiName = isHash(height) ? 'getBlockByHash' : 'getBlockByHeight'
                    const newBlock = await this.requester.send(`${MODULE_NAME}_${apiName}`, [height, !!withBody])
                    tempBlock = newBlock
                    callback(parseBlock(this.signer, newBlock, withBody))
                    return
                }
            }
            tempBlock = block
