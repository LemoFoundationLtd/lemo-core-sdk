            ...
            if (pendingBlocks.length) {
                ...
                if (Number(block.header.height) !== maxPblockHeight) {
                    ...
                    for (let i = maxPblockHeight; i < Number(block.header.height); i++) {
                        const newBlock =  this.requester.send(`${MODULE_NAME}_getBlockByHeight`, [i, !!withBody])

                        newBlock.then(value => {
                            pendingBlocks.some((item, index) => {
                                if (Number(value.header.height) === Number(item.header.height) + 1) {
                                    pendingBlocks.splice(index + 1, 0, value)
                                    return true
                                }
                            })
                        })
