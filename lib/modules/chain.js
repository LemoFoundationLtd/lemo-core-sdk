...
const apis = {
    ...
    watchBlock(withBody, callback) {
        const fetchBlock = (params) => {
            return this.requester.send('chain_getBlockByHeight', [params, !!withBody])
        }
        const watchHandler = block => {
            const newBlock = parseBlock(this.signer, block, withBody)
            processBlock(fetchBlock, this.signer, newBlock, callback)
        }
