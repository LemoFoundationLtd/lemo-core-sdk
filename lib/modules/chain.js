...
const apiList = {
     ...
     */
    getCurrentBlock: {
        call(stable, withBody) {
            const apiName = (typeof stable === 'undefined' || stable) ? 'latestStableBlock' : 'currentBlock'
            return this.requester.send(`${MODULE_NAME}_${apiName}`, [withBody]).then(parseBlock)
        },
    },
    /**
     ...
     */
    getBlock: {
        call(hashOrHeight, withBody) {
            const apiName = isHash(hashOrHeight) ? 'getBlockByHash' : 'getBlockByHeight'
            return this.requester.send(`${MODULE_NAME}_${apiName}`, [hashOrHeight, withBody]).then(parseBlock)
        },
    },
    /**
    ...
    watchBlock: {
        call(withBody, callback) {
            return this.requester.watch(`${MODULE_NAME}_latestStableBlock`, [withBody], callback)
        },
    },
}
