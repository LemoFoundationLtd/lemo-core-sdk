...
const apis = {
    ...
    watchBlock(withBody, callback) {
        const watchHandler = async block => {
            if (lastBlock) {
                const lastBlockHeight = Number(lastBlock.header.height) + 1
                if (lastBlockHeight !== Number(block.header.height)) {
                    for (let i = lastBlockHeight; i < Number(block.header.height); i++) {
