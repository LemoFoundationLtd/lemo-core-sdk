...
import Watcher from '../watchers/blocks_watcher'
...
const apis = {
    ...
    async getCurrentBlock(stable, withBody) {
        ...
        const block = await this.requester.send(`${CHAIN_NAME}_${apiName}`, [!!withBody])
    ...
    getCurrentHeight(stable) {
        ...
        return this.requester.send(`${CHAIN_NAME}_${apiName}`)
    },
    /**
    ...
    watchBlock(withBody, callback) {
        return  watcher.subscribe(this.requester, withBody, callback)
    },
    stopWatchBlock(callbacks) {
        if (this.requester === undefined) {
            throw new Error('can not use stopWatchBlock before using watchBlock')
        }
        callbacks.shift()
        watcher.unSubscribe(this.requester, callbacks)
    },
    /**
