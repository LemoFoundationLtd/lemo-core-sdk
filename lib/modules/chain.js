...
import {parseMoney, parseBlock, parseAccount} from '../network/data_parser'
...
const apis = {
    ...
    async getGenesis() {
        ...
        return parseBlock(this.signer, result)
    },
    /**
     ...
     * @return {Promise<object>}
     */
    async getCandidateList(address) {
        const result = await this.requester.send(`${MODULE_NAME}_getCandidateList`, [address])
     ...
     * @return {Promise<object>}
     */
    async getCandidateTop30(address) {
        const result = await this.requester.send(`${MODULE_NAME}_getCandidateTop30`, [address])
        return parseAccount(result).candidate
    },
}
