...
const apiList = {
     ...
     */
    getCurrentBlock: (requester, stable, withBody) => {
        const apiName = stable ? 'latestStableBlock' : 'currentBlock'
        return requester.send(`${MODULE_NAME}_${apiName}`, [withBody]).then(parseBlock)
    },
    /**
     ...
     * @return {Promise<object>}
     */
    getBlock: (requester, hashOrHeight, withBody) => {
        const apiName = isHash(hashOrHeight) ? 'getBlockByHash' : 'getBlockByHeight'
        return requester.send(`${MODULE_NAME}_${apiName}`, [hashOrHeight, withBody]).then(parseBlock)
    },
    /**
     ...
     */
    watchBlock: (requester, withBody, callback) => {
        return requester.watch(`${MODULE_NAME}_latestStableBlock`, [withBody], callback)
    },
}
