...
import {createTempAddress, decodeUtf8Hex} from '../utils'
...
const apis = {
     ...
     * @return {string}
     */
    signCreateTempAddress(privateKey, txConfig, userId, modifySignersInfo) {
        txConfig.to = createTempAddress(txConfig.from, userId)
        txConfig = checkChainID(txConfig, this.chainID)
        const tx = new ModifySignersTx(txConfig, modifySignersInfo)
        const weights = JSON.parse(decodeUtf8Hex(tx.data)).signers.reduce((item, index) => {
            return index.weight + item.weight
        })
        if (weights >= 100) {
            tx.signWith(privateKey)
            return JSON.stringify(tx.toJson())
        } else {
            throw new Error(errors.WeightNotReach())
        }
    },
    /**
