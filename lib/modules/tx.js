...
const apis = {
     ...
     * @param {string|number} address Account address
     * @param {number} start Index of transactions
     * @param {number} size The count of Transactions required
     * @return {Promise<object>}
     */
    async getTxListByAddress(address, start, size) {
        const result = await this.requester.send(`${MODULE_NAME}_getTxListByAddress`, [address, start, size])
        if (!result) {
            return null
        }
