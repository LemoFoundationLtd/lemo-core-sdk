...
import {parseTxRes, parseTxListRes, parseNumber} from '../network/data_parser'
...
const apis = {
     ...
     */
    readContract(contractAddress, data) {
        return this.requester.send(`${MODULE_NAME}_readContract`, [contractAddress, data])
    },
    /**
     ...
     * @param {number} txType The type of transaction
     * @param {string} data Smart contract execution code data
     * @return {Promise<number>}
     */
    async estimateGas(to, txType, data) {
        const gas = await this.requester.send(`${MODULE_NAME}_estimateGas`, [to, txType, data])
        return parseNumber(gas)
    },
}
