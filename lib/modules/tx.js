...
const apis = {
    ...
    signCreateTempAddress(privateKey, txConfig, userId) {
        txConfig.to = createTempAddress(txConfig.from, userId)
        txConfig = checkChainID(txConfig, this.chainID)
        const modifySignersInfo = {
            signers: [{
                address: txConfig.from,
                weight: 100,
            }],
        }
     ...
     * @param {string} privateKey The private key from sender account
     * @param {object} txConfig Transaction config
     * @param {object} boxTxInfo Box transaction information
     * @return {string}
     */
    signBoxTx(privateKey, txConfig, boxTxInfo) {
        txConfig = checkChainID(txConfig, this.chainID)
        const tx = new BoxTx(txConfig, boxTxInfo)
        tx.signWith(privateKey)
        return JSON.stringify(tx.toJson())
    },
    /**
     ...
     * @param {string} privateKey The private key from sender account
     * @param {object} txConfig Transaction config
     * @param {string} code Transaction config
     * @param {string} constructorArgs Transaction config
     * @return {string}
     */
    signContractCreation(privateKey, txConfig, code, constructorArgs) {
        txConfig = checkChainID(txConfig, this.chainID)
        const tx = new ContractCreationTx(txConfig, code, constructorArgs)
        tx.signWith(privateKey)
        return JSON.stringify(tx.toJson())
    },
    /**
